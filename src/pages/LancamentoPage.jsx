...
export default function LancamentoPage() {
  ...
  useEffect(() => {
    if (obraAtiva) {
      supabase.from('fases').select('id,nome,numero').eq('obra_id', obraAtiva.id).order('numero').then(r => setFases(r.data || []))
      supabase.from('momentos').select('id,nome,numero').eq('obra_id', obraAtiva.id).order('numero').then(r => setMomentos(r.data || []))
    }
  }, [obraAtiva?.id])
  ...
  async function handleSubmit(e, lancarOutro = false) {
    ...
    const { data: despesa, error } = await supabase.from('despesas').insert({
      ...
      momento_id: form.momento_id || null,
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="flex gap-2 mt-1">
                {['pendente','pago'].map(s => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => set('status_pagamento', s)}
                    className={cn(
          ...
          {success && (
            <div className="flex items-center gap-2 text-status-green text-sm bg-green-50 rounded-xl p-3">
              ...
              Despesa lançada! Fases e Momentos atualizados.
            </div>
          )}
        </form>
      </div>
